            ...
            <div class="px-4 py-4 sm:px-6">
              <div class="flex items-center justify-between">
                ...
                <div class="ml-2 flex-shrink-0 flex">
                  ...
                  <div v-if="!Boolean(item.specimen_collected)">
                    <button
                      @click="setCollected(item.id)"
                      type="button"
                  ...
                  <div v-else>
                    <button
                      @click="cancleCollected(item.id)"
                      type="button"
                      class="ml-2 inline-flex items-center px-3 py-2 border border-transparent shadow-sm text-sm leading-4 font-medium rounded-md text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                    >
                      <!-- Heroicon name: solid/beaker -->
                      <svg
                        class="-ml-0.5 mr-2 h-4 w-4"
...
export default {
  ...
  methods: {
    ...
    setCollected(myId) {
      this.$apollo.mutate({
        mutation: setCollectedDate,
        variables: {
          id: myId,
          specimen_collected: `${this.$dayjs().format(
            'YYYY-MM-DD'
          )}T${this.$dayjs().format('HH:mm:ss')}.000Z`,
        },
      })
    ...
    cancleCollected(myId) {
      this.$apollo.mutate({
        mutation: cancleCollectedDate,
        variables: {
          id: myId,
        },
      })
